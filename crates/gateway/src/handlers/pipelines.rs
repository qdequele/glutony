...
pub fn is_yaml(content_type: Option<&str>) -> bool {
    ...
    matches!(essence.as_str(), "application/x-yaml" | "application/yaml" | "text/yaml" | "text/x-yaml")
...
/// both fail).
pub fn parse_pipeline(body: &[u8], content_type: Option<&str>) -> Result<PipelineDefinition, GatewayError> {
    let text = std::str::from_utf8(body).map_err(|_| GatewayError::BadRequest("pipeline body must be UTF-8".into()))?;
    if text.trim().is_empty() {
        return Err(GatewayError::BadRequest("empty pipeline body".into()));
    }
    let mut def: PipelineDefinition = if is_yaml(content_type) {
        serde_yaml::from_str(text).map_err(|e| GatewayError::BadRequest(format!("invalid pipeline YAML: {e}")))?
    } else {
...
pub async fn create_pipeline(
    ...
    def.validate().map_err(|e| GatewayError::Invalid(e.to_string()))?;
    if def.builtin {
        return Err(GatewayError::Forbidden("user pipelines cannot be marked builtin".into()));
    }
...
pub async fn list_pipelines(
    ...
    Ok(Json(state.control_plane.list_pipelines(project_id.as_deref()).await?))
}
...
pub async fn get_pipeline(
    ...
    let project_id = resolve_project_id(&headers, &state.config);
    Ok(Json(state.control_plane.get_pipeline(&name, project_id.as_deref()).await?))
}
...
pub async fn delete_pipeline(
    ...
    let project_id = resolve_project_id(&headers, &state.config);
    state.control_plane.delete_pipeline(&name, project_id.as_deref()).await?;
...
"#;
    ...
    fn yaml_content_types() {
        for ct in ["application/x-yaml", "application/yaml; charset=utf-8", "text/yaml", "text/x-yaml", "application/vnd.x+yaml"] {
    ...
    fn parse_yaml_json_and_fallback() {
        ...
        assert!(matches!(parse_pipeline(b"   ", None), Err(GatewayError::BadRequest(_))));
        assert!(matches!(parse_pipeline(b"{", Some("application/x-yaml")), Err(GatewayError::BadRequest(_))));
    }
    ...
    async fn post_yaml_is_validated_forwarded_with_tenant_and_returns_201() {
        ...
        Mock::given(method("POST"))
            .and(path("/pipelines"))
            .and(body_partial_json(json!({"uid": "my-pdf", "project_id": "xxx"})))
    ...
    async fn post_without_context_is_global() {
        ...
        Mock::given(method("POST"))
            .and(path("/pipelines"))
            .respond_with(ResponseTemplate::new(201).set_body_json(json!({"uid": "g", "steps": [{"id": "a", "plugin": "chunker"}]})))
            .mount(&server)
            .await;
        let (app, _) = test_app(&server, GatewayConfig::default()).await;
        let body = json!({"uid": "g", "steps": [{"id": "a", "plugin": "chunker"}]}).to_string();
        let req = Request::post("/pipelines").header(CONTENT_TYPE, "application/json").body(Body::from(body)).unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
    ...
    async fn invalid_pipeline_is_422_with_message_and_bad_text_is_400() {
        ...
        let req = Request::post("/pipelines").header(CONTENT_TYPE, "application/json").body(Body::from("{{{")).unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
        assert!(server.received_requests().await.unwrap().is_empty(), "nothing forwarded");
    }
    ...
    async fn control_plane_422_passes_through() {
        ...
        Mock::given(method("POST"))
            .and(path("/pipelines"))
            .respond_with(ResponseTemplate::new(422).set_body_json(json!({"error": "unknown plugin \"nope\"", "code": "unknown_plugin"})))
            .mount(&server)
            .await;
        let (app, _) = test_app(&server, GatewayConfig::default()).await;
        ...
        let json = json_body(resp).await;
        assert_eq!(json["code"], "invalid_pipeline");
        assert!(json["error"].as_str().unwrap().contains("unknown plugin \"nope\""));
    }
    ...
    async fn list_get_delete_are_proxied_with_tenant_and_403_passes_through() {
        ...
        Mock::given(method("DELETE"))
            .and(path("/pipelines/builtin.pdf"))
            .respond_with(ResponseTemplate::new(403).set_body_json(json!({"error": "built-in pipelines cannot be deleted"})))
            .mount(&server)
            .await;
        Mock::given(method("DELETE"))
        ...
        let req = Request::get("/pipelines").header("x-meili-project-id", "xxx").body(Body::empty()).unwrap();
        let resp = app.clone().oneshot(req).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        assert_eq!(json_body(resp).await.as_array().unwrap().len(), 1);

        let resp = app.clone().oneshot(Request::get("/pipelines/builtin.pdf").body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        assert_eq!(json_body(resp).await["uid"], "builtin.pdf");

        let resp = app.clone().oneshot(Request::get("/pipelines/ghost").body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::NOT_FOUND);

        let resp = app.clone().oneshot(Request::delete("/pipelines/builtin.pdf").body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::FORBIDDEN);
        assert_eq!(json_body(resp).await["code"], "forbidden");

        let resp = app.oneshot(Request::delete("/pipelines/mine").body(Body::empty()).unwrap()).await.unwrap();
