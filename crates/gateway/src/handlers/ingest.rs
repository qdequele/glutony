...
pub async fn submit_one(
    ...
    let (pipeline, index_pattern) = match selection {
        PipelineSelection::Auto { explicit } => {
            match state
                .control_plane
                .resolve(&mime, filename.as_deref(), ctx.project_id.as_deref(), explicit.as_deref())
                .await
            {
    ...
    let target_index = resolve_index(&mut ctx, index_pattern.as_deref(), &mime, &state.config.default_index);
    let job_id = Uuid::new_v4();

    let input = match payload {
        IngestPayload::File(blob) => state.blob.stage_upload(job_id, blob, state.config.inline_max_bytes).await?,
        ...
        IngestPayload::Batch(_) => return Err(GatewayError::BadRequest("nested batches are not supported".into())),
    };

    let pipeline_uid = pipeline.uid.clone();
    let wf_input = PipelineWorkflowInput { job_id, pipeline, input, context: ctx.clone() };
    ...
    Ok(IngestResponse { job_id, pipeline_used: pipeline_uid, target_index, status: "queued".into() })
}
...
pub async fn submit_batch(
    ...
    for item in items {
        match submit_one(state, item, ctx.clone(), selection).await {
            ...
            Err(e) => jobs.push(BatchEntry::Error(ErrorBody { error: e.to_string(), code: e.code().to_string() })),
        }
    }
    BatchResponse { jobs }
}
...
pub async fn ingest(
    ...
    let explicit = query_param(&query, "pipeline").map(str::to_string).or(extracted.pipeline);
    if matches!(extracted.payload, IngestPayload::Batch(_)) {
        return Err(GatewayError::BadRequest("`items` batches must be sent to POST /ingest/batch".into()));
    }
    let resp = submit_one(&state, extracted.payload, ctx, &PipelineSelection::Auto { explicit }).await?;
...
mod tests {
    ...
    async fn multipart_pdf_starts_workflow_with_context_pipeline_and_index() {
        ...
        match &wf.input {
            PluginInput::Bytes(Blob { data, mime, filename }) => {
        ...
        let resolve = requests.iter().find(|r| r.url.path() == "/internal/resolve").unwrap();
        ...
        // job was cached
        let created = requests.iter().find(|r| r.url.path() == "/internal/jobs").unwrap();
    ...
    async fn large_upload_is_staged_as_ref() {
        ...
        mount_resolve(&server, "builtin.pdf", None).await;
        mount_jobs_ok(&server).await;
        let cfg = GatewayConfig { inline_max_bytes: 16, ..GatewayConfig::default() };
        let (app, starter) = test_app(&server, cfg).await;

        let (ct, body) = multipart_body("file", "big.pdf", "application/pdf", PDF_MAGIC);
        let resp = app.oneshot(standalone_request("/ingest", ct, body)).await.unwrap();
        assert_eq!(resp.status(), StatusCode::ACCEPTED);
        let inputs = starter.inputs();
        match &inputs[0].input {
            PluginInput::Ref(ContentRef::Staged { uri, mime, filename }) => {
    ...
    async fn index_pattern_from_trigger_overrides_query_and_mime_default_applies() {
        ...
        let resp = app.clone().oneshot(standalone_request("/ingest?index=q", ct, body)).await.unwrap();
        let json = json_body(resp).await;
    ...
    async fn json_url_and_documents_bodies() {
        ...
        let resp = app.clone().oneshot(standalone_request("/ingest", "application/json", body.into())).await.unwrap();
        assert_eq!(resp.status(), StatusCode::ACCEPTED);
        let json = json_body(resp).await;
        assert_eq!(json["target_index"], "from-body");
        match &starter.inputs()[0].input {
            PluginInput::Ref(ContentRef::Url { url, mime, filename }) => {
        ...
        let body = json!({"documents": [{"id": "1", "title": "Hello", "content": "world"}]}).to_string();
        let resp = app.oneshot(standalone_request("/ingest", "application/json", body.into())).await.unwrap();
        assert_eq!(resp.status(), StatusCode::ACCEPTED);
    ...
    async fn no_pipeline_for_mime_is_415_and_missing_context_is_400() {
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(404).set_body_json(json!({"error": "no pipeline matches"})))
            .mount(&server)
            .await;
        let (app, starter) = test_app(&server, GatewayConfig::default()).await;
    ...
    async fn explicit_pipeline_param_unknown_is_404() {
        ...
        let (ct, body) = multipart_body("file", "a.pdf", "application/pdf", PDF_MAGIC);
        let resp = app.oneshot(standalone_request("/ingest?pipeline=ghost", ct, body)).await.unwrap();
    ...
    async fn temporal_failure_is_502() {
        ...
        assert!(server.received_requests().await.unwrap().iter().all(|r| r.url.path() != "/internal/jobs"));
    }
    ...
    async fn batch_continues_past_failures() {
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({"pipeline": sample_pipeline("builtin.pdf", None)})))
            .mount(&server)
            .await;
        Mock::given(method("POST"))
            .and(path("/internal/resolve"))
            .and(body_partial_json(json!({"mime": "application/octet-stream"})))
            .respond_with(ResponseTemplate::new(404).set_body_json(json!({"error": "no pipeline matches"})))
            .mount(&server)
            .await;
        mount_jobs_ok(&server).await;
        ...
        .to_string();
        let resp = app.oneshot(standalone_request("/ingest/batch", "application/json", body.into())).await.unwrap();
        assert_eq!(resp.status(), StatusCode::ACCEPTED);
        let json = json_body(resp).await;
    ...
    async fn body_over_limit_is_413() {
        let server = MockServer::start().await;
        let cfg = GatewayConfig { max_upload_mb: 1, ..GatewayConfig::default() };
        let (app, _) = test_app(&server, cfg).await;
        let big = vec![b'a'; 1024 * 1024 + 10];
        let resp = app.oneshot(standalone_request("/ingest", "text/plain", big)).await.unwrap();
